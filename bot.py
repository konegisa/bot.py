...
async def slowmode(interaction: discord.Interaction, saniye: int):
    ...
    await interaction.response.send_message(f"Yavaş mod {saniye} saniyeye ayarlandı!")
...
async def rolver(interaction: discord.Interaction, member: discord.Member, rol: discord.Role):
    ...
    await interaction.response.send_message(f"{member} adlı kullanıcıya {rol} verildi!")
...
async def rolal(interaction: discord.Interaction, member: discord.Member, rol: discord.Role):
    ...
    await interaction.response.send_message(f"{member} adlı kullanıcıdan {rol} alındı!")
...
async def on_message(msg):
    ...
    if any(kufur in msg.content.lower() for kufur in KUFURLER):
        await msg.delete()
        await msg.channel.send(f"{msg.author.mention} küfür yasaktır!", delete_after=3)
